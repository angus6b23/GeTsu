...
const getHolidays = async (year, month, countryId, raw = false) => {
    ...
    if (memory[`holiday-${year}-${month}-${countryId}`])
        return memory[`holiday-${year}-${month}-${countryId}`]
    const res = await fetch(
        `https://date.nager.at/api/v3/PublicHolidays/${year}/${countryId}`
    )
    const data = await res.json()
    const holidays = data
        .filter((holiday) =>
            holiday.date.includes(`${year}-${month.toString().padStart(2, 0)}`)
        )
        .map((holiday) => {
            return {
                ...holiday,
                day: parseInt(holiday.date.replace(/\d+\-\d+\-/, '')),
            }
        })
...
const getRegions = async (countryId) => {
    ...
    if (memory[`region-${countryId}`]) {
        ...
        let regions = data.reduce((acc, curr) => {
            if (curr.counties) {
                curr.counties.forEach((county) =>
                    !acc.includes(county) ? acc.push(county) : null
                )
            }
            return acc
        }, [])
